...
function inferPlatformFromName(fileName) {
  ...
  if (normalizedFileName.endsWith(".pkg") || normalizedFileName.endsWith(".dmg")) return "macos";
  if (
...
function normalizeRegion(endpoint, rawRegion) {
  ...
  if (!isR2Endpoint) return region;
  if (r2AllowedRegions.has(region)) return region;

  console.warn(`[release:r2] WARNING: S3_REGION='${region}' is not valid for R2 endpoint. Falling back to 'auto'.`);
  return "auto";
}
...
function parseBucketAndPrefix(rawBucket) {
  ...
  if (!cleaned) throw new Error("S3_BUCKET is empty after trimming");
...
function isRetryableUploadError(error) {
  ...
  if (error.$retryable || error.retryable || error.name === "TimeoutError") return true;
  ...
  return ["socket hang up", "connection reset", "timed out", "bad record mac", "streaming request", "network error"].some((pattern) => message.includes(pattern));
}
...
async function uploadObjectWithRetries({
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      console.warn(`[release:r2] Transient upload error on ${objectKey} (attempt ${attempt}/${maxAttempts}): ${formatUploadError(error)}`);
...
async function notifyDownloadSync({ product, platform, latestManifestUrl, latestPayload }) {
  ...
  const body = {
    source: "relyyconvert-release-upload",
    productSlug: product,
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      console.warn(`[release:r2] Download catalog sync notify failed (attempt ${attempt}/3): ${error.message}`);
      if (attempt < 3) {
        await sleep(delayMs);
      }
    }
...
async function main() {
  ...
  if (explicitPrefix && impliedPrefix) {
    console.warn("[release:r2] WARNING: S3_BUCKET contains a path and S3_PREFIX is set. Using S3_PREFIX and ignoring path part from S3_BUCKET.");
  }
  ...
  if (basePrefix) console.log("[release:r2] Prefix:", basePrefix);
  ...
  const latestPayload = { ...metadata, url: publicUrl || null };
  await uploadObjectWithRetries({
    s3,
    bucket,
    objectKey: latestKey,
  ...
  if (publicUrl) console.log("[release:r2] Public URL:", publicUrl);
  const latestPublicUrl = buildPublicUrl(process.env.S3_PUBLIC_URL, bucket, latestKey);
  if (latestPublicUrl) console.log("[release:r2] Latest manifest:", latestPublicUrl);
...
main().catch((error) => {
  if (error.message.includes("The specified bucket does not exist")) {
    console.error("[release:r2] ERROR: The bucket does not exist. Set S3_BUCKET to the actual bucket name and use S3_PREFIX for folder path (or S3_BUCKET=bucket/path).");
  } else {
